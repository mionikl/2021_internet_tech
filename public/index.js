...
class AppModel {
  static async getTasklists() {
    const tasklistsRes = await fetch('http://localhost:1313/tasklists');
  ...
  static async addTask({
    ...
    const result = await fetch(
      `http://localhost:1313/tasklists/${tasklistId}/tasks`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
  ...
  static async editTask({
    ...
    const result = await fetch(
      `http://localhost:1313/tasklists/${tasklistId}/tasks/${taskId}`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
  ...
  static async moveTask({
    ...
    const result = await fetch(
      `http://localhost:1313/tasklists/${fromTasklistId}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите адрес билборда:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новый адрес билборда', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Билборд по адресу '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить адрес билборда';
